...
async def watch(tourname):
    response = await get_response(f'http://180140_gateway-api:6543/watch/{tourname}')
...
async def watch_end(tourname):
    response = await get_response(f'http://180140_gateway-api:6543/watch_end/{tourname}')
    logger.info(f'WATCH END {tourname}')
    return jsonify({'State': 'Ok'})

@activities.route('/tours/<tourname>/watch_check/')
async def watch_check(tourname):
    response = await get_response(f'http://180140_gateway-api:6543/watch_check/{tourname}')
