...
@reservations.route('/tours/<tourname>/finish_reservation/', methods=['GET', 'POST'])
@login_required
async def fbuy(tourname):
    if request.method == 'POST':
        card_number = request.form.get('card_number')
        price = request.form.get('price')
        room = request.form.get('room')
    ...
    if response['result']:
        data = {
            'tourid': tourname,
            'price': response['result'][3],
            'room': response['result'][4],
            'adults': response['result'][6],
            'ch3': response['result'][7],
            'ch10': response['result'][8],
            'ch18': response['result'][9],
            'response': True
        }
    else:
        data = {
    ...
    return await arender('buy-finish.html', data)
